...
import { quotations, fmtRp } from "@/lib/customer-mock";
...
const Quotations = () => (
  <>
    <PageHeader
      ...
      action={<Button variant="hero" asChild><Link to="/quotation"><Plus className="h-4 w-4" />Buat Baru</Link></Button>}
    />
    ...
    <div className="rounded-3xl bg-card border border-border/40 shadow-soft overflow-hidden">
      <div className="p-4 border-b border-border/60 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Cari quotation atau produk..." className="pl-9 rounded-full" />
        </div>
          ...
          <tbody>
            {quotations.map((q) => (
              <tr key={q.id} className="border-t border-border/40 hover:bg-secondary/30 transition-colors">
                ...
                <td className="px-5 py-4 hidden md:table-cell text-muted-foreground">{q.createdAt}</td>
                <td className="px-5 py-4 text-right font-bold">{fmtRp(q.total)}</td>
                <td className="px-5 py-4 text-center">
                  <span className={`text-[10px] px-2 py-1 rounded-full font-semibold ${tone[q.status]}`}>{q.status}</span>
                </td>
                <td className="px-5 py-4 text-right">
                  <Button variant="ghost" size="sm">Detail <ArrowRight className="h-3.5 w-3.5" /></Button>
                </td>
              </tr>
            ))}
          </tbody>
