    ...
    <>
      ...
      <Card className="border-border/60">
        <CardContent className="p-0">
          {isLoading ? (
            <div className="p-5 text-sm text-muted-foreground">Memuat pricing rules...</div>
          ) : error ? (
            <div className="p-5 text-sm text-destructive">Gagal memuat pricing rules.</div>
          ) : rules.length === 0 ? (
            <div className="p-5 text-sm text-muted-foreground">Belum ada pricing rule.</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {rules.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-xs">{r.apply_to || "—"}</TableCell>
